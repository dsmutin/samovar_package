  ...
  for(iter in 1:N) {
    ...
    while(length(cl_done) < samovar_base$samovar_data$cluster_n()) {
      ...
      if(is.na(init)) {
        ...
        X <- samovar_base$samovar_data$get(from_to[2])
        Y <- samovar_base$samovar_data$get(from_to[1])
        yval <- results$get_cluster(from_to[1]) %>% mean

        cluster_init_level <- get_newval(X, Y, yval,
      ...
      if(!is.na(init)) {
        ...
        if(cluster_len > 1) {
          ...
          if (nrow(prediction_graph) > 0) for (i in seq_along(prediction_graph)) {
            X <- df_cl[prediction_graph$to[i],]
            Y <- df_cl[prediction_graph$from[i],]
            yval <- species_abundance[prediction_graph$from[i]]
            species_abundance[prediction_graph$to[i]] <-
              get_newval(X = X, Y = Y, yval = yval,
