...
## Get new value ----
get_newval_general <- function(X, Y, model,
                               yval,
                               mode = "glm",
                               summarise_cluster,
                               glm_calc){
  ...
  if (mode == "glm") {
    glmxy <- glm_calc(X, Y, model)
    yval <- summarise_cluster(yval)

    if(is.numeric(glmxy)) return(glmxy*yval)

    xval <- try(predict(glmxy, newdata = data.frame(Y = yval)),
                silent = T) #!!!!!!
    if(is.character(xval)) xval <- 0
  } else {
    warning("Not implemented")
  }
